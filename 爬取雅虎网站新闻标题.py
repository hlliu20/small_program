...
from google_tran import GoogleTranslator
# from youdao_tran_net import tran_youdao
...
def save(title_list):
    ...
    file_name_1 = "E:/python_workolace/news_everyday_homework/split_3/{}_{}_{}_{}.txt".format(mon, day, hor, mine)
    file_name_2 = "E:/python_workolace/news_everyday_homework/news_en_from_10_26.txt"
    title = "{}_{}_{}_{}".format(mon, day, hor, mine)
    row = ""
    with open(file_name_2, "a", encoding='utf-8') as fa:
        fa.write(title + "\n")
        order = 1
        for it in title_list:
            tran = translater.translate(it)
            # tran_yd = tran_youdao(it)
            row += str(order) + ". " + it + "\n    " + tran + "。\n"
            # row += str(order) + ". " + it + "\n    " + tran + "。\n    "+ tran_yd +"。\n"
            order += 1
            print(it + "\n" + tran)
